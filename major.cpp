...
int main(int argc, char** argv){
    ...
    if (argc != 4) {
        cout << "Please enter all 3 inputs (k, k', # of threads)!" << endl; 
    } 
    else {
      k = atoi(argv[1]);
      kk = atoi(argv[2]);
    ...
    // parallel computing
    #pragma omp parallel shared (result, n, a, b)
    {
      #pragma omp single 
      {
        result = strassen(n, a, b);
      }
    }
    ...
    cout << "K: " << k << ", K': "<< kk << ", Threads: " << num_threads << ", Time: " << total_time << endl;
